...
void mmu_client_create(struct mmu_client *c)/*{{{*/
{
	...
	pager_create(c->pid);
	snprintf(msg, 96, "create pid %d", id);
	...
	strncat(rep.pmem_fn, mmu->pmem_fn, MMU_PROTO_PATH_MAX-1);
...
int main(int argc, char **argv) {/*{{{*/
	...
	#endif
	memset(id2pid, 255, UINT8_MAX * sizeof(pid_t));
